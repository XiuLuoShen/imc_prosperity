...
def passive_trades_bananas(state: TradingState, product, orders: List[Order], acceptable_price, curr_pos: int, long_vol_avail: int, short_vol_avail: int, bids, asks, bid_sizes, ask_sizes):
    if long_vol_avail > 0:
        post_px = int(np.floor(acceptable_price))
        if post_px-4 > bids[0] and curr_pos >= -19:
            post_sz = long_vol_avail
            long_vol_avail -= post_sz
            orders.append(AlgoOrder(product, post_px-4, 'BUY', post_sz, note='P3'))
...
def passive_trades_pearls(state: TradingState, product, orders: List[Order], acceptable_price, curr_pos: int, long_vol_avail: int, short_vol_avail: int, bids, asks, bid_sizes, ask_sizes):
...
def alpha_trade(state: TradingState, product):
    ...
    if product == 'BANANAS':
        passive_trades_bananas(state, product, orders, acceptable_price, curr_pos, long_vol_avail, short_vol_avail, bids, asks, bid_sizes, ask_sizes)
    if product == 'PEARLS':
        passive_trades_pearls(state, product, orders, acceptable_price, curr_pos, long_vol_avail, short_vol_avail, bids, asks, bid_sizes, ask_sizes)   
